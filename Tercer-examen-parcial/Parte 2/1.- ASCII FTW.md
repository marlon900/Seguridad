## Objetivo de nivel
Este programa ha construido la bandera utilizando valores ascii hexadecimales. Identifique el texto de la bandera desensamblando el programa. Puedes descargar el archivo desde [aquí](https://artifacts.picoctf.net/c/506/asciiftw).

## Solución
- Entramos al archivo utilizando `Ghydra`.
- Buscamos el `main`, y vemos que el apartado de la bandera terminan con un número hexadecimal.
``` bash
...
```
- Guardamos estos datos en un archivo y realizamos la siguiente sentencia:
``` shell
┌──(kali㉿kali)-[~/Downloads]
└─$ cat doc | cut -d "," -f2 | xxd -r -p 
